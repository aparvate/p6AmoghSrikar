...
// Helper to allocate a new block and update the inode's block mapping
static int allocate_and_map_block(struct wfs_inode *inode, int block_offset, int disk_index, int logical_block_num) {
    ...
    if (superblock->raid_mode == 0) {
        // RAID 0: Mirror the block mapping across all disks
        for (int i = 0; i < diskNum; i++) {
        ...
        // RAID 1: Update all disks with the allocated block
        for (int i = 0; i < diskNum; i++) {
            char *disk = (char *)disks[i];
...
static void write_to_block(const char *buf, size_t bytes_written, size_t bytes_to_write, size_t block_start_offset, uint32_t block_address, int disk_index) {
    if (superblock->raid_mode == 0) {
        // RAID 0: Write only to the target disk
        void *block_ptr = (char *)disks[disk_index] + block_address + block_start_offset;
        memcpy(block_ptr, buf + bytes_written, bytes_to_write);
    } else {
        // RAID 1: Write to all disks
        for (int i = 0; i < diskNum; i++) {
            void *block_ptr = (char *)disks[i] + block_address + block_start_offset;
            memcpy(block_ptr, buf + bytes_written, bytes_to_write);
        }
    }
}

// Helper to handle indirect blocks
static int handle_indirect_block(struct wfs_inode *inode, int disk_index) {
    if (inode->blocks[IND_BLOCK] == 0) {
...
static int wfs_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
    ...
    while (bytes_written < size) {
        ...
        if (block_offset < D_BLOCK) {
            if (inode->blocks[block_offset] == 0) {
                if (allocate_and_map_block(inode, block_offset, disk_index, logical_block_num) < 0) {
                    return -ENOSPC;
                }
            }
        ...
        // Handle indirect blocks
            ...
            if (handle_indirect_block(inode, disk_index) < 0) {
                return -ENOSPC;
            }
            ...
            if (indirect_block[indirect_offset] == 0) {
                if (allocate_and_map_block(inode, indirect_offset, disk_index, logical_block_num) < 0) {
                    return -ENOSPC;
                }
            }
