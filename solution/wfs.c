...
struct wfs_inode *locate_inode(const char *path) {
  struct wfs_inode *inode_table = (struct wfs_inode *)((char *)disk_images[0] + ((struct wfs_sb)disk_images[0]->i_blocks_ptr));
  size_t num_inodes = (struct wfs_sb)disk_images[0]->num_inodes;
